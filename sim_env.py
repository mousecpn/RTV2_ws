...
import glob
import random
from msg import Pose, PoseArray, PoseStamped, Twist, KeyCommand
...
class BMINavigationSim(object):
    ...
    def ee_pose(self):
        pos, orn = p.getBasePositionAndOrientation(self.avatar)
        pose = Pose()
...
if __name__=="__main__":
    ...
    for e in range(start_e, start_e+epsidoes):
        ...
        while True:
            key_commands = [command_msg.twist.linear.x, command_msg.twist.linear.y, 0]

            velo = [0,0]
            if key_commands[0]!=0.0 or key_commands[1]!=0.0:
                current_pos = avatar.ee_pose()
                messagepos = [current_pos.position.x, current_pos.position.y] 
                cur_velo = [key_commands[1], 0, key_commands[0]]
                zmqmsg.SendMessage(
                        avatar.publisher,
                            "AvatarInfo",
                            {
