...
import numpy as np
from .model_utils import ModeKeys,rgetattr,rsetattr,CustomLR,exp_anneal,sigmoid_anneal,unpack_RNN_state,run_lstm_on_variable_length_seqs,mutual_inf_mc
from .dynamics import SingleIntegrator
from .discrete_latent import DiscreteLatent
from .gmm3d import GMM3D
from .gmm2d import GMM2D
from .attention import AdditiveAttention,MapEncoder
...
class MultimodalGenerativeCVAE(nn.Module):
    def __init__(self,
        ...
        self.entropy_ub = None
        self.entropy_lb = None
    ...
    def create_node_models(self):
        ...
        if self.hyperparams['edge_encoding']:
            ...
            for context_type in self.context_types:
                self.add_submodule('/{}_edge_encoder'.format(context_type),
                                model=nn.Linear(
                                        in_features= self.pred_state_length, #+ self.hyperparams['enc_rnn_dim_history'],
                                        out_features=self.hyperparams['enc_rnn_dim_edge']))
                if self.hyperparams['edge_influence_combine_method'] == 'attention':
                    self.add_submodule('/{}_edge_influence_encoder'.format(context_type),
            ...
            self.eie_output_dims = self.hyperparams['enc_rnn_dim_edge_influence'] * len(self.context_types)
        
        if self.hyperparams['map_encoding']:
            self.add_submodule('/map_encoder', 
                               model=MapEncoder(self.hyperparams['enc_rnn_dim_edge_influence']))
    ...
    def obtain_encoded_tensors(self,
        ...
        if self.hyperparams['edge_encoding']:
            ...
            for context_type in context.keys():
        ...
        if self.hyperparams['edge_encoding']:
            x_concat_list += node_edges_encoded
    ...
    def p_y_xz(self, mode, x, n_s_t0, z_stacked, prediction_horizon,
        ...
        for j in range(ph):
            ...
            self.entropy_ub = float(self.cal_entropy_ub(gmm, self.latent.p_dist.logits).detach().cpu().data)
            self.entropy_lb = float(self.cal_entropy_lb(gmm, self.latent.p_dist.logits).detach().cpu().data)
    ...
    def encode_edge(self,
        ...
        for c_idx in range(len(contexts)): #batch size
            ...
            output = self.node_modules['/{}_edge_encoder'.format(context_type)](context)
    ...
    def predict(self,
        ...
        y_dist, v_dist, our_sampled_future = self.p_y_xz(mode, x, n_s_t0, z,
                                            prediction_horizon,
                                            num_samples,
                                            num_components,
                                            gmm_mode,
