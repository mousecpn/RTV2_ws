...
import time
import numpy as np 
import predictor_assistance.Utils as Utils
import sys
import predictor_assistance.Csp as Csp
...
class PotentialFunction:
    ...
    """
    def __init__(self, threshold_distance, neta, k_a, escape_constant, obstacle_position, goal, escape_points, print_file):
    ...
    def getTotalPotential(self):
        ...
        for obs in range(len(self._obs_position)):
            distance = Utils.computeDistance(self._actual_positions[0:3], self._obs_position[obs])
            # if(distance < min_distance):
            #     min_distance = distance
            #     idmin = obs
                 #check if the control point is under influence of potential field and compute repulsive force
            if(distance <= self._p0):
                rep_force_ee += self.getRepulsivePotential(distance, obs)
        # if(min_distance <= self._p0):
        #     rep_force_ee = self.getRepulsivePotential(min_distance, idmin)
        ...
        if((self.isThereAnObstacle()) and len(self._escape_points)>0):
            # print("ESCAPE")
    ...
    def getAttractivePotential(self):
        ...
        for i in range(len(self._goal_position)):
            distance = Utils.computeDistance(ee_position[0:3], self._goal_position[i])
    ...
    def getRepulsivePotential(self, distance_from_obstacle, idobs):
        """
        ...
        st = 1/(self._p0)
        #consider only linear component
        rep_f[0:3] = self._neta * (ft - st) * (ft * ft) * ((self._actual_positions[0:3] - self._obs_position[idobs][0:3])/p_x)
    ...
    #     p_x = distance_from_obstacle
    #     rep_f = np.zeros(self._LEN_TWIST_VECTOR)
    #     p = self._actual_positions[:3]
    #     p_o = self._obs_position[idobs]
    #     ft = 1/(p_x)
    #     st = 1/(self._p0)
    #     #consider only linear component
    #     rep_f[0:3] = self._neta * (p - p_o)
    ...
    def computeC2(self):
        ...
        ee_position = self._actual_positions[:3]
    ...
    def isThereAnObstacle(self):
        ...
        idx_goal_max_prob = self._goal_distribution.tolist().index(prob_max)
