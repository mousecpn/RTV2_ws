...
class GMM2D(td.Distribution):
    ...
    def __init__(self, log_pis, mus, log_sigmas, corrs):
        ...
        self.corrs = corrs#.reshape(list(corrs.shape[:-1]) + [self.components,])  # [..., N]
    ...
    @classmethod
    def from_log_pis_mus_cov_mats(cls, log_pis, mus, cov_mats, kf=False):
        dim = mus.shape[-1]
    ...
    def rsample(self, sample_shape=torch.Size(), output_mode=False):
        """
        ...
        mvn_samples = (self.mus +
                       torch.squeeze(
                           ...
                           dim=-1))
        # component_cat_samples = self.pis_cat_dist.sample(sample_shape) # sample mode for every timestep
        component_cat_samples = self.pis_cat_dist.sample(sample_shape)[...,:1] # sample mode for every trajectory
        selector = torch.unsqueeze(to_one_hot(component_cat_samples, self.components), dim=-1)
        if output_mode is True:
            return torch.sum(mvn_samples*selector, dim=-2), selector
